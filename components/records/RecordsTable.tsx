            ...
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr
                    key={r.id}
                    className={`border-t ${selectionMode ? "cursor-pointer" : ""} ${
                  ...
                  >
                    ...
                    <td 
                      className="p-2 whitespace-nowrap overflow-hidden text-ellipsis" title={bdMap[r.bd_id] ?? r.bd_id}>
                        {bdMap[r.bd_id] ?? r.bd_id}
                    </td>


                    <td className="p-2 whitespace-nowrap overflow-hidden text-ellipsis" title={levelMap[r.bd_level_id] ?? r.bd_level_id}>
                      {levelMap[r.bd_level_id] ?? r.bd_level_id}
                    </td>

                    <td className="p-2 whitespace-nowrap overflow-hidden text-ellipsis" title={r.customer_name}>
                      {r.customer_name}
                    </td>

                    <td className="p-2 whitespace-nowrap overflow-hidden text-ellipsis" title={customerTypeMap[r.customer_type_id] ?? r.customer_type_id}>
                      {customerTypeMap[r.customer_type_id] ?? r.customer_type_id}
                    </td>

                    <td className="p-2 whitespace-nowrap overflow-hidden text-ellipsis" title={pointTypeMap[r.point_type_id] ?? r.point_type_id}>
                      {pointTypeMap[r.point_type_id] ?? r.point_type_id}
                    </td>

                    <td className="p-2">{r.points?.toLocaleString("en-US")}</td>

                    <td className="p-2">{r.money?.toLocaleString("en-US") ?? ""}</td>
